...
class Game(object):
    def main(self, screen):
		...
		learner = Learn(self.matris.lines, self.matris.current_tetromino, self.matris.board, MATRIX_WIDTH, MATRIX_HEIGHT)

        while True:
            try:
                timepassed = clock.tick(50)
                if self.matris.update((timepassed / 1000.) if not self.matris.paused else 0):
                	learn.update(self.matris.lines, self.matris.current_tetromino, self.matris.board)
                	#print("update")
                	self.redraw()
            except GameOver:
                return
